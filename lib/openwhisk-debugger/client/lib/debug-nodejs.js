...
const fs = require('fs'),
      ...
      doNotInsertBreakpoints = process.env.NO_DEBUGGER_BREAKPOINTS
...
exports._debug = function debugNodeJS(message, ws, echoChamberNames, done, commandLineOptions, eventBus) {
    ...
    if (startOfMethodBody >= 0) {
        ...
        if (!doNotInsertBreakpoints) {
            code += '\n    debugger;    // <-- inserted by the OpenWhisk debugger to break on main\n\n'
        } else {
            console.log('Not inserting a debugger breakpoint')
        }
        code += originalCode.substring(paren + 1);
        ...
        try {
            ...
            function trySpawnWithBrowser(webPort, debugPort) {
                ...
                if (isOldNodeJS) {
                    ...
                    console.log('Spawning inspect-process')
